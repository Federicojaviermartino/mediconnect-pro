...
class InsuranceService {
  constructor() {
    ...
    if (this.mockMode) {
      logger.warn('No insurance API keys configured. Insurance features will use mock responses.', {
        service: 'insurance',
        feature: 'initialization'
      });
    } else {
      logger.info('Insurance integration service initialized', {
        service: 'insurance',
  ...
  async verifyEligibility(patientData, insuranceProvider) {
    try {
      ...
      if (!provider.enabled) {
        logger.warn('Insurance provider API not configured, using mock data', {
          service: 'insurance',
  ...
  async requestPreAuthorization(appointmentData, serviceCode) {
    try {
      ...
      logger.error('Error requesting pre-authorization', {
        service: 'insurance',
        operation: 'pre-authorization',
        error: error.message,
        stack: error.stack
      });
      throw error;
